...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    std::cout<<"Euler Integration: test　end"<<std::endl;
...
//读取生成的imu数据并用imu动力学模型对数据进行计算，最后保存imu积分以后的轨迹，
//用来验证数据以及模型的有效性。
void IMU::testImuMiddlePoint(std::string src, std::string dist)
{
    std::vector<MotionData>imudata;
    LoadPose(src,imudata);
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        MotionData imupose = imudata[i];
        MotionData imupose_last = imudata[i-1];//previous data

        //delta_q = [1 , 1/2 * thetax , 1/2 * theta_y, 1/2 * theta_z]
        Eigen::Quaterniond dq;
        /*Eigen::Vector3d dtheta_half =  imupose.imu_gyro * dt /2.0; //ideal data no bias
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        /// 中值积分
        Eigen::Vector3d dtheta_half =  (imupose_last.imu_gyro + imupose.imu_gyro) / 2.0 * dt /2.0; //ideal -> no bias
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        //Update
        Eigen::Quaterniond Qwb_new = Qwb*dq;
        Eigen::Vector3d acc_w = (Qwb*imupose_last.imu_acc + gw + Qwb_new*imupose.imu_acc + gw) / 2.0;  // aw = Rwb * ( acc_body - acc_bias ) + gw
        Qwb = Qwb_new;
        Vw = Vw + acc_w * dt;
    ...
    std::cout<<"Midpoint Integration: test　end"<<std::endl;
