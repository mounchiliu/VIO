    ...
    :bStart_backend(true)
{
    ...
    ofs_pose.open("/home/meng7/HW7/dataset/pose_output.txt",fstream::app | fstream::out);
    ofs_pose.clear();
    if(!ofs_pose.is_open())
    {
...
void System::PubImageData(double dStampSec, Mat &img)
{
    ...
    trackerData[0].readImage(img, dStampSec);//process images (e.g. get feature points)
        ...
        {//Records
            ...
            for (unsigned int j = 0; j < ids.size(); j++)
            {
                if (trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    feature_points->velocity_x_of_point.push_back(pts_velocity[j].x);//speed of movement of feature points in pixels
...
void System::ProcessBackEnd()
{
    ...
    {
        // cout << "1 process()" << endl;
        //img data with corresponding imu data
        vector<pair<vector<ImuConstPtr>, ImgConstPtr>> measurements;
        
        unique_lock<mutex> lk(m_buf);
        con.wait(lk, [&] {
            return (measurements = getMeasurements()).size() != 0;//process defined 'buf' for 1. imu 2. image
        });
