...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double header)
{
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))//find how much the image moved to determine the KF -> margin / slide window
    ...
    if (ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            ...
            if (initial_ex_rotation.CalibrationExRotation(corres, pre_integrations[frame_count]->delta_q, calib_ric))
            {
                ...
                ric[0] = calib_ric;
                RIC[0] = calib_ric;
                ESTIMATE_EXTRINSIC = 1; // Then we can do the initilizaiton
            }
        }
    }
    ...
    {
        TicToc t_solve;
        solveOdometry(); // estimate camera pose
...
void Estimator::solveOdometry()
{
    ...
    {
        TicToc t_tri;
        f_manager.triangulate(Ps, tic, ric); //Triangulation
        //cout << "triangulation costs : " << t_tri.toc() << endl;        
        backendOptimization();//Back-end
    }
}
...
void Estimator::backendOptimization()
{
    TicToc t_solver;
    // 借助 vins 框架，维护变量
    vector2double(); //Pre-process data
    // 构建求解器
    problemSolve(); //Least-square optimization
    // 优化后的变量处理下自由度
    double2vector();//After optimization, the frame will drift.
    //We need to calculate the position before optimization
    //(Use the first frame to calculate how much it drift)
    //for all the frames within the slide window
