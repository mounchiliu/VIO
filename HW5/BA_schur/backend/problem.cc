    ...
    for (auto &edge: edges_) {
        ...
        for (size_t i = 0; i < verticies.size(); ++i) {
            ...
            for (size_t j = i; j < verticies.size(); ++j) {
                ...
                //block(start row, start col, index_row, index_col);
                //upper
...
//        delta_x_ = PCGSolver(H, b_, H.rows() * 2);
        ...
        // TODO:: home work. 完成矩阵块取值，Hmm，Hpm，Hmp，bpp，bmm //vio 5 slides pp4.
         MatXX Hmm = Hessian_.block(reserve_size,reserve_size, marg_size, marg_size);//bottom right
         MatXX Hpm = Hessian_.block(0,reserve_size, reserve_size, marg_size);//upper right
         //MatXX Hmp = Hessian_.block(reserve_size,0, marg_size, reserve_size);//bottom left
         MatXX Hmp = Hpm.transpose();
...
void Problem::TestMarginalize() {
    ...
    H_marg.block(reserve_size-dim,0,reserve_size - idx - dim,reserve_size) = temp_rows;//Exchange

    // 将 col i 移动矩阵最右边
    Eigen::MatrixXd temp_cols = H_marg.block(0, idx, reserve_size, dim);
    Eigen::MatrixXd temp_rightCols = H_marg.block(0, idx + dim, reserve_size, reserve_size - idx - dim);//cols right to the margin col
    H_marg.block(0, idx, reserve_size, reserve_size - idx - dim) = temp_rightCols;
    H_marg.block(0, reserve_size - dim, reserve_size, dim) = temp_cols; //Exchange
