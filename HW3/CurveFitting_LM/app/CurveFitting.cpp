...
public:
    ...
    CurveFittingVertex(): Vertex(4) {}  // abc: 三个参数， Vertex 是 3 维的
...
public:
    ...
    {
        Vec4 abc = verticies_[0]->Parameters();  // 估计的参数
//        residual_(0) = std::exp( abc(0)*x_*x_ + abc(1)*x_ + abc(2) ) - y_;  // 构建残差
        residual_(0) =  abc(0)*x_*x_* x_ + abc(1)*x_* x_ + abc(2)* x_ + abc(3) - y_; //y' - y
    }

    // 计算残差对变量的雅克比
    virtual void ComputeJacobians() override
    {
        Vec4 abc = verticies_[0]->Parameters();
//        double exp_y = std::exp( abc(0)*x_*x_ + abc(1)*x_ + abc(2) );
        //double y = abc(0)*x_*x_ + abc(1)*x_ + abc(2);

        Eigen::Matrix<double, 1, 4> jaco_abc;  // 误差为1维，状态量 3 个，所以是 1x3 的雅克比矩阵
//        jaco_abc << x_ * x_ * exp_y, x_ * exp_y , 1 * exp_y;
        jaco_abc << x_ * x_ * x_, x_* x_ , 1* x_, 1;
...
//    int N = 100;                          // 数据点
    ...
    // 设定待估计参数 a, b, c初始值
    vertex->SetParameters(Eigen::Vector4d (0.,0.,0.,0.));
    // 将待估计的参数加入最小二乘问题
...
//        double y = std::exp( a*x*x + b*x + c ) + n;
        double y =  a*x*x*x + b*x*x + c*x + d + n;
    ...
    std::cout << "1.0,  2.0,  1.0 , 1.0" << std::endl;
